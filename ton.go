...
func subscribeToTransactions(token string) {
	...
	if err != nil {
		fmt.Println("Lỗi khi tạo tệp:", err)
		return
	}
	defer file.Close()
	...
	for {
		err := streamingAPI.SubscribeToTransactions(context.Background(),
			...
			func(data tonapi.TransactionEventData) {
				...
				// Tạo một map với key "hashtx" và giá trị là chuỗi Golang
				data_tx := map[string]string{"hashtx": data.TxHash}
				// Chuyển đổi map thành JSON
				jsonData, err := json.Marshal(data_tx)
				if err != nil {
				...
				fmt.Println("Dữ liệu đã được lưu vào tệp output.json.")
			})
		if err != nil {
